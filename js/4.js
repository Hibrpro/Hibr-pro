...
(function () {
            ...
            case "object":
                if (mixed_val == null) return "N;";
                    ...
                    for (var i = 0; i < mixed_val.length; i++) {
                        ...
                        if (ser) map.push(shaha_serialize(idxobj.idx) + ser);
                    }
                    ...
                    for (var prop in mixed_val) {
                        ...
                        if (ser) props.push(shaha_serialize(prop) + ser);
                    }
    ...
    window.shaha_get_date_difference = function (dt1) {
        ...
        } else var a2 = shaha_parse_date(dt2);
    ...
    window.shaha_color_picker = function () {
        ...
        tbl += '</tr></thead><tbody>' + '<tr style="height:60px;">' + '<td colspan="8" id="' + id + '_colorPreview" style="vertshahal-align:middle;text-align:center;border:1px solid #000;background:#fff;">' + '<input style="width:55px;color:#000;border:1px solid rgb(0, 0, 0);padding:5px;background-color:#fff;font:11px Arial, Helvetshaha, sans-serif;" maxlength="7" />' + '<a href="#" id="' + id + '_moreColors" class="iColorPicker_moreColors"></a>' + '</td>' + '<td colspan="8" id="' + id + '_colorOriginal" style="vertshahal-align:middle;text-align:center;border:1px solid #000;background:#fff;">' + '<input style="width:55px;color:#000;border:1px solid rgb(0, 0, 0);padding:5px;background-color:#fff;font:11px Arial, Helvetshaha, sans-serif;" readonly="readonly" />' + '</td>' + '</tr></tbody></table>';
    ...
    window.shaha_del_cookie = function (name, path, domain) {
        ...
        if (shaha_get_cookie(name)) document.cookie = name + '=' + ((path) ? ';path=' + path : '') + ((domain) ? ';domain=' + domain : '') + ';expires=Thu, 01-Jan-1970 00:00:01 GMT';
    };
    ...
    window.shaha_debug_object = function (obj) {
        ...
        if (obj !== null && (typeof (obj) == 'object' || typeof (obj) == 'array')) {
            for (var prop in obj) {
                ...
                if (level < recursive && (typeof (obj[prop]) == 'object' || typeof (obj[prop]) == 'array') && obj[prop] != obj) dispStr += addStr + prop + '=' + shaha_debug_object(obj[prop], recursive, showMethods, level + 1);
                else {
                    try {
                        curStr = "" + obj[prop];
                    } catch (e) {
...
jQuery(document).ready(function () {
    ...
    function shaha_ready_actions() {
        ...
        if (document.documentElement.className.indexOf(SHAHA_STORAGE['site_scheme']) == -1) document.documentElement.className += ' ' + SHAHA_STORAGE['site_scheme'];
        ...
        jQuery('.menu_mobile').on('click', 'li a, li a .open_child_menu', function (e) {
            ...
            if (!jQuery(this).hasClass('open_child_menu') && shaha_is_local_link($a.attr('href'))) jQuery('.menu_mobile_close').trigger('click');
        ...
        jQuery('.nav-links-more a').on('click', function (e) {
            ...
            if (panel.length == 0) {
                jQuery.get(location.href, {
                    ...
                    shaha_loadmore_add_items(jQuery('.content .posts_container').eq(0), jQuery(response).find('.content .posts_container > article,' + '.content .posts_container > div[class*="column-"],' + '.content .posts_container > .masonry_item'));
                });
            } else {
                jQuery.post(SHAHA_STORAGE['ajax_url'], {
                    ...
                    try {
                        ...
                        rez = {
                            error: SHAHA_STORAGE['strings']['ajax_error']
                        };
            ...
            function shaha_loadmore_add_items(container, items) {
                ...
                else SHAHA_STORAGE['load_more_link_busy'] = false;
                jQuery(window).trigger('scroll');
    ...
    function shaha_stretch_sidemenu() {
        ...
        var toc_items_height = jQuery(window).height() - shaha_fixed_rows_height(true, false) - jQuery('.menu_side_wrap .sc_layouts_logo').outerHeight() - toc_items.length;
    ...
    function shaha_check_layout() {
        ...
        if (SHAHA_STORAGE['mobile_device'] || shaha_browser_is_mobile()) jQuery('body').addClass('mobile_device');
    }
        ...
        if (sb.length > 0) {
            if (content.css('float') == 'none') {
                ...
                if (sb_height < content_height && scroll_offset + top_panel_fixed_height > content_top) {
                    ...
                    if (scroll_dir == 0) scroll_dir = SHAHA_STORAGE['scroll_dir_last'];
        ...
        if (SHAHA_STORAGE['use_mediaelements'] && cont.find('audio:not(.inited),video:not(.inited)').length > 0) {
            if (window.mejs) {
                ...
                cont.find('audio:not(.inited),video:not(.inited)').each(function () {
                    ...
                    if (jQuery(this).parents('.mejs-mediaelement').length == 0 && jQuery(this).parents('.wp-block-video').length === 0 && !jQuery(this).hasClass('wp-block-cover__video-background') && jQuery(this).parents('.elementor-background-video-container').length === 0 && (SHAHA_STORAGE['init_all_mediaelements'] || (!jQuery(this).hasClass('wp-audio-shortcode') && !jQuery(this).hasClass('wp-video-shortcode') && !jQuery(this).parent().hasClass('wp-playlist')))) {
    ...
    function shaha_tabs_ajax_content_loader(panel, page, oldPanel) {
        ...
        } else panel.find('> *').addClass('shaha_tab_content_remove');
    ...
    function shaha_comments_validate(form) {
        ...
        var comments_args = {
            error_message_text: SHAHA_STORAGE['strings']['error_global'],
            error_message_show: true,
            error_message_time: 4000,
            error_message_class: 'shaha_messagebox shaha_messagebox_style_error',
            ...
            rules: [{
                ...
                min_length: {
                    value: 1,
                    message: SHAHA_STORAGE['strings']['text_empty']
                },
                max_length: {
                    value: SHAHA_STORAGE['comment_maxlength'],
                    message: SHAHA_STORAGE['strings']['text_long']
                }
            }]
        };
        if (form.find('.comments_author input[aria-required="true"]').length > 0) {
            comments_args.rules.push({
                ...
                min_length: {
                    value: 1,
                    message: SHAHA_STORAGE['strings']['name_empty']
                },
                max_length: {
                    value: 60,
                    message: SHAHA_STORAGE['strings']['name_long']
                }
            });
        }
        if (form.find('.comments_email input[aria-required="true"]').length > 0) {
            comments_args.rules.push({
                ...
                min_length: {
                    value: 1,
                    message: SHAHA_STORAGE['strings']['email_empty']
                },
                max_length: {
                    value: 60,
                    message: SHAHA_STORAGE['strings']['email_long']
                },
                mask: {
                    value: SHAHA_STORAGE['email_mask'],
                    message: SHAHA_STORAGE['strings']['email_not_valid']
                }
            });
        }
...
jQuery(document).on('action.init_hidden_elements', function (e, cont) {
    ...
    if (SHAHA_STORAGE['button_hover'] && SHAHA_STORAGE['button_hover'] !== 'default') {
        jQuery('button:not(.search_submit):not([class*="sc_button_hover_"]):not([class*="tribe-events-"]),\ .theme_button:not([class*="sc_button_hover_"]),\ .sc_theme_button_style_1 .elementor-button:not([class*="sc_button_hover_"]),\ .sc_theme_button_style_2 .elementor-button:not([class*="sc_button_hover_"]),\ .sc_theme_button_style_3 .elementor-button:not([class*="sc_button_hover_"]),\ .sc_button:not([class*="sc_button_simple"]):not([class*="sc_button_hover_"]):not(.sc_price_item_link):not(.sc_button_services),\ .sc_form_field button:not([class*="sc_button_hover_"]),\ .post_item .more-link:not([class*="sc_button_hover_"]),\ .trx_addons_hover_content .trx_addons_hover_links a:not([class*="sc_button_hover_"]),\ .shaha_tabs .shaha_tabs_titles li a:not([class*="sc_button_hover_"]),\ .hover_shop_buttons .icons a:not([class*="sc_button_hover_style_"]),\ .mptt-navigation-tabs li a:not([class*="sc_button_hover_style_"]),\ .edd_download_purchase_form .button:not([class*="sc_button_hover_style_"]),\ .edd-submit.button:not([class*="sc_button_hover_style_"]),\ .widget_edd_cart_widget .edd_checkout a:not([class*="sc_button_hover_style_"]),\ .slider_prev:not([class*="sc_button_hover_style_"]),\ .slider_next:not([class*="sc_button_hover_style_"]),\ #buddypress a.button:not([class*="sc_button_hover_"])\ ').addClass('sc_button_hover_just_init sc_button_hover_' + SHAHA_STORAGE['button_hover']);
        if (SHAHA_STORAGE['button_hover'] !== 'arrow') {
            jQuery('input[type="submit"]:not([class*="sc_button_hover_"]),\ input[type="button"]:not([class*="sc_button_hover_"]),\ .vc_tta-color-grey.vc_tta-style-classic .vc_tta-tab > a:not([class*="sc_button_hover_"]),\ .tribe-events-button:not([class*="sc_button_hover_"]),\ #tribe-bar-views .tribe-bar-views-list .tribe-bar-views-option a:not([class*="sc_button_hover_"]),\ .tribe-bar-mini #tribe-bar-views .tribe-bar-views-list .tribe-bar-views-option a:not([class*="sc_button_hover_"]),\ .tribe-events-cal-links a:not([class*="sc_button_hover_"]),\ .tribe-events-sub-nav li a:not([class*="sc_button_hover_"]),\ .isotope_filters_button:not([class*="sc_button_hover_"]),\ .trx_addons_scroll_to_top:not([class*="sc_button_hover_"]),\ .sc_promo_modern .sc_promo_link2:not([class*="sc_button_hover_"]),\ .post_item_single .post_content .post_meta .post_share .social_item .social_icon:not([class*="sc_button_hover_"]),\ .slider_container .slider_prev:not([class*="sc_button_hover_"]),\ .slider_container .slider_next:not([class*="sc_button_hover_"]),\ .sc_slider_controller_titles .slider_controls_wrap > a:not([class*="sc_button_hover_"]),\ .tagcloud > a:not([class*="sc_button_hover_"]),\ .wp-block-tag-cloud > a:not([class*="sc_button_hover_"]),\ .post_item_single .post_content .post_meta .post_share .social_item .social_icon:not([class*="sc_button_hover_style_"])\ ').addClass('sc_button_hover_just_init sc_button_hover_' + SHAHA_STORAGE['button_hover']);
        }
        ...
        jQuery('.trx_addons_hover_content .trx_addons_hover_links a:not([class*="sc_button_hover_style_"]),\ .sc_action_item.with_bg_color .sc_action_item_link\ ').addClass('sc_button_hover_just_init sc_button_hover_style_inverse');
        jQuery('.mc4wp-form button,\ .widget_tag_cloud a:not([class*="sc_button_hover_style_"]),\ .wp-block-tag-cloud a:not([class*="sc_button_hover_style_"]),\ .sidebar .trx_addons_tabs .trx_addons_tabs_titles li a:not([class*="sc_button_hover_style_"]),\ .vc_tta-color-grey.vc_tta-style-classic .vc_tta-tab > a:not([class*="sc_button_hover_style_"]),\ .trx_addons_tabs .trx_addons_tabs_titles li a:not([class*="sc_button_hover_style_"]),\ .post_type_donation .sc_donations_form_submit\ ').addClass('sc_button_hover_just_init sc_button_hover_style_hover');
        jQuery('.shaha_tabs .shaha_tabs_titles li a:not([class*="sc_button_hover_style_"])\ ').addClass('sc_button_hover_just_init sc_button_hover_style_alterbd');
        jQuery('.sc_button.color_style_dark:not([class*="sc_button_simple"]):not([class*="sc_button_hover_style_"]),\ .trx_addons_video_player.with_cover .video_hover:not([class*="sc_button_hover_style_"])\ ').addClass('sc_button_hover_just_init sc_button_hover_style_dark');
        jQuery('').addClass('sc_button_hover_just_init sc_button_hover_style_extra');
        jQuery('.sc_button.color_style_link2:not([class*="sc_button_simple"]):not([class*="sc_button_hover_style_"]),\ .sc_theme_button_style_2 .elementor-button:not([class*="sc_button_hover_style_"])\ ').addClass('sc_button_hover_just_init sc_button_hover_style_link2');
        jQuery('.sc_button.color_style_link3:not([class*="sc_button_simple"]):not([class*="sc_button_hover_style_"]),\ .sc_theme_button_style_3 .elementor-button:not([class*="sc_button_hover_style_"])\ ').addClass('sc_button_hover_just_init sc_button_hover_style_link3');
        setTimeout(function () {
            jQuery('.sc_button_hover_just_init').removeClass('sc_button_hover_just_init');
        }, 500);
        jQuery('.mejs-controls button,\ .mfp-close,\ .sc_button_bg_image,\ .hover_shop_buttons a,\ button.pswp__button,\ .sc_theme_button_none .elementor-button,\ .sc_layouts_row_type_narrow .sc_button\ ').removeClass('sc_button_hover_' + SHAHA_STORAGE['button_hover']);
    }
});
(function () {
    "use strict";
